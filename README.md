...
**_Base URL (via API Gateway)_** : `/api/accounts`
...
**_Base URL (via API Gateway)_** : `/api/transactions`
...
- ###### Validation Rules
  ...
  | Field                | Type   | Required | Description              |
  | -------------------- | ------ | -------- | ------------------------ |
  | `sourceAccount`      | String | Yes      | Must be valid and active |
  | `destinationAccount` | String | Yes      | Must be valid and active |
...
**Note** :-
The config files on GitHub (which the Config Server reads) are configured for Docker.
If you want to run the services without Docker, you must change the Config Server to use local native configuration instead of the GitHub repository.
...
Comment out these properties (Git-based config):

```yml
spring.cloud.config.server.git.uri=https://github.com/sharmavikas4/Banking-System-Simulator-Config-Files
spring.cloud.config.server.git.clone-on-start=true
spring.cloud.config.server.git.default-label=main
```

And uncomment these properties (Native/local config):
...
When running with Docker → Config Server reads configuration files from GitHub.

When running without Docker → Config Server should read from local files inside /config folder of the ConfigServer project.
